...
BEGIN_ALLOC_FUNCTION(MCMB_DA){
  bitratesJS = readBitrates("./networks/bitratesMB_C_L.json");
  ...
  std::map<std::string, std::pair<float,float>> bands = {{"C", std::make_pair(0, 343)}, {"L", std::make_pair(343, 823)}};

  // Declared and assigned the order in which the bands will be used
  std::map<std::string , std::vector<std::string>> setOfBands = {{"set_1",{"C", "L"}}, {"set_2",{"L", "C"}}}; // Definimos el orden de las bandas en dos sets, el set 2 es un set de equilibrio
