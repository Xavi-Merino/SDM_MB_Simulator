...
BEGIN_ALLOC_FUNCTION(MCMB_DA){
  bitratesJS = readBitrates("./networks/bitratesMB_C_L_S_E.json");
  ...
  std::map<std::string, std::pair<float,float>> bands = {{"E", std::make_pair(0, 1135)}, {"S", std::make_pair(1135, 1895)}, {"C", std::make_pair(1895, 2239)}, {"L", std::make_pair(2239, 2719)}};
...
int main(int argc, char* argv[]) {
  ...
  // lambdas[i] = (i + 1) * 1000;
  // }
  for (int lambda = 52; lambda < 100; lambda++) {
